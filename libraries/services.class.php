...
class services {
	...
	public function __construct() {
		$this->paslaugos_lentele = 'paslaugos';
		...
		$query = "  SELECT COUNT(`{$this->sutartys_lentele}`.`nr`) AS `kiekis`
					FROM `{$this->paslaugos_lentele}`
						INNER JOIN `{$this->paslaugu_kainos_lentele}`
							ON `{$this->paslaugos_lentele}`.`id`=`{$this->paslaugu_kainos_lentele}`.`fk_Paslaugaid`
						INNER JOIN `{$this->uzsakytos_paslaugos_lentele}`
							ON `{$this->paslaugu_kainos_lentele}`.`fk_Paslaugaid`=`{$this->uzsakytos_paslaugos_lentele}`.`fk_Paslaugos_kaina`
						INNER JOIN `{$this->sutartys_lentele}`
							ON `{$this->uzsakytos_paslaugos_lentele}`.`fk_Sutartisnr`=`{$this->sutartys_lentele}`.`nr`
	...
	{
		$query = "SELECT id FROM {$this->paslaugos_lentele} ORDER BY id DESC LIMIT 1";
		mysql::query($query);
		$data = mysql::select($query);
		if(!empty($data))
		{
	...
	{
		$query = "SELECT fk_Paslaugaid FROM {$this->paslaugu_kainos_lentele} ORDER BY fk_Paslaugaid DESC LIMIT 1";
		mysql::query($query);
		$data = mysql::select($query);
		if(!empty($data))
		{
	...
	public function deleteServicePrice($serviceId, $galiojaNuo, $kaina) {
		...
		$query = "  DELETE FROM `{$this->paslaugu_kainos_lentele}`
					WHERE `fk_Paslaugaid`='{$serviceId}' AND `galioja_nuo`='{$galiojaNuo}' AND `kaina`='{$kaina}'";
		mysql::query($query);
	}
	...
	public function getOrderedServices($dateFrom, $dateTo) {
		...
		$query = "  SELECT `id`,
					...
					FROM `{$this->paslaugos_lentele}`
						INNER JOIN `{$this->uzsakytos_paslaugos_lentele}`
							ON `{$this->paslaugos_lentele}`.`id`=`{$this->uzsakytos_paslaugos_lentele}`.`fk_Paslaugaid`
						INNER JOIN `{$this->sutartys_lentele}`
							ON `{$this->uzsakytos_paslaugos_lentele}`.`fk_Sutartisnr`=`{$this->sutartys_lentele}`.`nr`
					{$whereClauseString}
